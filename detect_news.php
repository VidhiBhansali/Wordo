...
$query="select word from news where frequency='".$frequency."'";
$result = mysql_query($query) or die($query."<br/><br/>".mysql_error());
$row = mysql_fetch_array($result);
$news = $row['word'];
...
$left_sub5 = $art[$i-4].$space.$art[$i-3].$space.$art[$i-2].$space.$art[$i-1].$space.$news;	
$right_sub5 = $news.$space.$art[$i+1].$space.$art[$i+2].$space.$art[$i+3].$space.$art[$i+4];
...
$left_sub4 = $art[$i-3].$space.$art[$i-2].$space.$art[$i-1].$space.$news;	
...
$left_sub3 = $art[$i-2].$space.$art[$i-1].$space.$news;	
$right_sub3 = $news.$space.$art[$i+1].$space.$art[$i+2];
...
if ($total_pmi_sub5f > $total_pmi_5f)
    ...
    $string = $string.$news.$special.$space;
	}
else 
	{
